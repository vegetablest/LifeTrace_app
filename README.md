...
LifeTrace 是一个基于 AI 的智能生活记录系统，通过自动截图、OCR 文本识别和多模态搜索技术，帮助用户记录和检索日常活动。系统支持传统关键词搜索、语义搜索和多模态搜索，提供强大的生活轨迹追踪能力。

## 核心功能

- **自动截图记录**：定时自动截取屏幕内容，记录用户活动
- **智能 OCR 识别**：使用 RapidOCR 提取截图中的文本内容
- **多模态搜索**：支持文本、图像和语义搜索
...
#### 1. Web API 层 (`lifetrace/server.py`)

基于 FastAPI 构建的 RESTful API 服务，提供以下主要端点：
...
- **向量数据库管理**
  - `GET /api/vector-stats` - 向量数据库统计
  - `POST /api/vector-sync` - 同步向量数据库
  - `POST /api/vector-reset` - 重置向量数据库

#### 2. 数据模型层 (`lifetrace/models.py`)
...
- **OCRProcessor**: 基于 RapidOCR 的文本识别
- 支持多种图像格式
- 批量处理能力
- 结果缓存机制
- 与向量服务集成
...
##### 6.2 多模态向量服务 (`lifetrace/multimodal_vector_service.py`)
...
- **FileProcessor**: 文件监控和处理
- **ScreenshotHandler**: 截图文件事件处理
- 异步处理队列
- 文件变更监听
- 批量处理优化
...
```
截图生成 → 文件监控 → OCR处理 → 向量化 → 存储
    ↓           ↓         ↓        ↓       ↓
定时任务    文件事件   文本提取  嵌入生成  数据库
    ↓           ↓         ↓        ↓       ↓
多屏支持    队列处理   RapidOCR  CLIP模型 SQLite
                                          ↓
                                    向量数据库
                                   (ChromaDB)
```
...
#### 单独启动 Web 服务
```bash
python -m lifetrace.server --port 8840
```
...
1. **添加新的搜索算法**：扩展 `vector_service.py`
2. **支持新的 OCR 引擎**：修改 `ocr.py`
3. **添加新的 API 端点**：扩展 `server.py`
4. **自定义数据模型**：修改 `models.py`
...
### 调试模式
```bash
python -m lifetrace.server --debug
```
