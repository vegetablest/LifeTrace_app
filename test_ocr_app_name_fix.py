...
def test_ocr_app_name_detection():
    ...
    if not ocr_processor.is_available():
        print("❌ OCR引擎不可用，请安装 rapidocr-onnxruntime")
        return False
    
    # 测试用例：不同的应用截图文件
    test_cases = [
        ("Snipaste_2024-01-15_14-30-25.png", "Snipaste截图测试"),
        ("chrome_webpage_capture.png", "Chrome浏览器截图"),
    ...
    if success_count > 0:
        ...
        print(f"   应用名称准确率: {reasonable_rate:.1f}%")
...
def test_database_query():
    ...
    try:
        with db_manager.get_session() as session:
            from lifetrace.models import Screenshot
            
            # 查询最近的10个截图记录
            recent_screenshots = session.query(Screenshot).order_by(Screenshot.created_at.desc()).limit(10).all()
            
            if not recent_screenshots:
                print("📭 数据库中暂无截图记录")
                return
            
            print(f"📊 最近 {len(recent_screenshots)} 个截图的应用分布:")
            app_counts = {}
            
            for screenshot in recent_screenshots:
                # 正确访问模型属性
                app_name = getattr(screenshot, 'app_name', None) or '未知'
...
if __name__ == "__main__":
    try:
        # 测试OCR应用名称检测
        success = test_ocr_app_name_detection()
        ...
        if success:
            print("\n✅ 应用名称检测修复验证通过！")
            print("💡 现在外部截图文件应该能够正确识别应用名称，而不是统一显示为'外部工具'。")
        else:
            print("\n❌ 应用名称检测修复验证失败！")
