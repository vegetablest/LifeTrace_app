...
def backup_current_data():
    ...
    try:
        ...
        if db_path.exists():
            ...
            logger.info(f"数据库已备份到: {backup_db_path}")
        ...
        for vector_dir in vector_dirs:
            if vector_dir.exists():
                ...
                logger.info(f"向量数据库已备份到: {backup_vector_dir}")
...
def reset_sqlite_database():
    ...
    try:
        ...
        logger.info("SQLite数据库重置完成")
        return True
        
    except Exception as e:
        logger.error(f"重置SQLite数据库失败: {e}")
        return False

def reset_screenshot_files():
    """删除所有截图文件"""
    try:
        logger.info("开始删除截图文件...")
        ...
        if screenshots_dir.exists():
            ...
            for file_path in screenshots_dir.glob("*.png"):
                file_path.unlink()
                logger.debug(f"已删除截图: {file_path}")
    ...
    try:
        ...
        if screenshots_dir.exists():
            ...
            if len(screenshot_files) == 0:
                ...
                logger.warning(f"⚠️ 仍有 {len(screenshot_files)} 个截图文件")
...
def main():
    ...
    # 不进行备份，直接重置
    logger.info("跳过备份，直接开始重置")
    
    success_count = 0
    total_steps = 4
    
    # 1. 重置SQLite数据库
    if reset_sqlite_database():
        success_count += 1
