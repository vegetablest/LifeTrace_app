#!/usr/bin/env python3
"""
重置LifeTrace数据库工�?同时重置SQLite和向量数据库
"""
...
def reset_sqlite_database():
    ...
    try:
        # 获取数据库路�?        db_path = config.database_path
        print(f"数据库路�? {db_path}")
        
        # 删除数据库文�?        if os.path.exists(db_path):
            os.remove(db_path)
...
def reset_vector_database():
    ...
    try:
        ...
        if not vector_service.is_enabled():
            print("ℹ️  向量数据库未启用，跳过重�?)
            return True
        
        # 重置向量数据�?        success = vector_service.reset()
...
def show_status():
    ...
    # SQLite数据�?    db_path = config.database_path
    db_exists = os.path.exists(db_path)
    print(f"  SQLite数据�? {'存在' if db_exists else '不存�?} ({db_path})")
    
    # 向量数据�?    vector_db_path = Path(config.vector_db_persist_directory)
    vector_exists = vector_db_path.exists()
    print(f"  向量数据�? {'存在' if vector_exists else '不存�?} ({vector_db_path})")
    
    # 截图目录
    screenshots_dir = Path(config.screenshots_dir)
    if screenshots_dir.exists():
        file_count = len(list(screenshots_dir.glob("*")))
        print(f"  截图文件: {file_count} �?)
    else:
        print(f"  截图目录: 不存�?)
...
def main():
    ...
    operations = [
        ...
        ("重置SQLite数据�?, reset_sqlite_database),
        ("清理向量数据库文�?, clean_vector_files),
        ("重置向量数据�?, reset_vector_database),
        ("清理日志文件", clean_logs),
    ]
    ...
    print(f"\n总计: {success_count}/{len(results)} 个操作成�?)
    
    if success_count == len(results):
        print(f"\n🎉 数据库重置完成！")
        print(f"\n接下来可�?")
        print(f"  1. 重新初始�? lifetrace init")
