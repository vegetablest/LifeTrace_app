...
def test_sync_service_status():
    ...
    try:
        ...
        logger.info(f"同步服务管理器状�?")
        logger.info(f"  运行状�? {status.get('manager_running', False)}")
        logger.info(f"  启动时间: {status.get('start_time', 'N/A')}")
        logger.info(f"  统计信息: {status.get('stats', {})}")
        
        # 显示各个服务状�?        services = status.get('services', {})
...
def test_force_consistency_check():
    ...
    try:
        # 执行强制一致性检�?        result = sync_service_manager.force_consistency_check()
        
        logger.info(f"一致性检查结�?")
        if 'success' in result:
            logger.info(f"  执行状�? {'成功' if result['success'] else '失败'}")
        
        if 'result' in result:
            check_result = result['result']
            logger.info(f"  检查时�? {check_result.get('check_time', 'N/A')}")
            logger.info(f"  孤立数据库记�? {check_result.get('orphaned_db_records', 0)}")
            logger.info(f"  孤立文件: {check_result.get('orphaned_files', 0)}")
            logger.info(f"  清理记录�? {check_result.get('cleaned_records', 0)}")
...
def test_config_values():
    ...
    try:
        ...
        logger.info(f"  数据库路�? {config.database_path}")
        
        # 检查同步服务配�?        sync_configs = [
            ('enable_file_monitor', '文件监控启用'),
            ('enable_consistency_check', '一致性检查启�?),
            ('file_monitor_delay', '文件监控延迟'),
            ('consistency_check_interval', '一致性检查间�?),
        ...
        # 检查目录是否存�?        screenshots_dir = Path(config.screenshots_dir)
...
def test_service_restart():
    ...
    try:
        # 获取当前状�?        status_before = sync_service_manager.get_status()
        logger.info(f"重启前状�? {status_before.get('manager_running', False)}")
        
        if status_before.get('manager_running', False):
            # 尝试重启所有服�?            logger.info("尝试重启同步服务...")
            sync_service_manager.restart_all()
            
            # 等待重启完成
            time.sleep(2)
            
            # 获取重启后状�?            status_after = sync_service_manager.get_status()
            logger.info(f"重启后状�? {status_after.get('manager_running', False)}")
...
def main():
    ...
    tests = [
        ("配置值测�?, test_config_values),
        ("同步服务状态测�?, test_sync_service_status),
        ("强制一致性检查测�?, test_force_consistency_check),
    ...
    if passed == total:
        ...
        logger.warning(f"�?{total - passed} 个测试失�?)
        return 1
