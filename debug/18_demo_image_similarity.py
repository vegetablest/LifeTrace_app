    ...
    for query in queries:
        ...
        if results:
            for i, result in enumerate(results, 1):
                ...
                print(f"{i}. OCR ID: {ocr_id} | 相似�? {similarity:.4f} | 应用: {app_name}")
                if text_content:
...
def demo_similarity_analysis(calculator: TextImageSimilarityCalculator):
    ...
    # 统计相似度分�?    high_sim = [r for r in results if r['similarity'] >= 0.7]
    ...
    print(f"📈 相似度分布统�?")
    print(f"🟢 高相似度 (�?.7):   {len(high_sim)} �?)
    print(f"🟡 中等相似�?(0.5-0.7): {len(medium_sim)} �?)
    print(f"🟠 低相似度 (0.3-0.5):   {len(low_sim)} �?)
    print(f"🔴 很低相似�?(<0.3):   {len(very_low_sim)} �?)
    ...
    for i, result in enumerate(results, 1):
        ...
        print(f"{i:2d}. {level} 相似�? {similarity:.4f} (距离: {distance:.4f})")
...
def demo_application_analysis(calculator: TextImageSimilarityCalculator):
    ...
    for app_name, app_results in sorted(app_groups.items()):
        ...
        print(f"\n🔸 {app_name} ({len(app_results)} 个结�? 平均相似�? {avg_similarity:.4f})")
        
        # 显示该应用的�?个结�?        for i, result in enumerate(sorted(app_results, key=lambda x: x['similarity'], reverse=True)[:3], 1):
            similarity = result['similarity']
            text_content = result.get('text_content', '')[:40]
            print(f"  {i}. 相似�? {similarity:.4f} | 文本: {text_content}{'...' if len(result.get('text_content', '')) > 40 else ''}")
...
def demo_performance_test(calculator: TextImageSimilarityCalculator):
    ...
    for i, query in enumerate(queries, 1):
        ...
        print(f"{i}. 查询: '{query}' | 时间: {query_time:.3f}s | 结果: {len(results)} �?)
    
    avg_time = total_time / len(queries)
    print(f"\n📊 性能统计:")
    print(f"总查询时�? {total_time:.3f}s")
    print(f"平均查询时间: {avg_time:.3f}s")
    print(f"总结果数: {total_results} �?)
    print(f"平均结果�? {total_results / len(queries):.1f} �?查询")
...
def demo_database_stats(calculator: TextImageSimilarityCalculator):
    ...
    try:
        # 图像向量数据库统�?        if calculator.multimodal_service.image_vector_db:
            collection = calculator.multimodal_service.image_vector_db.collection
            total_images = collection.count()
            print(f"📷 图像向量数据�? {total_images} 个图�?)
        else:
            print("📷 图像向量数据�? 未初始化")
        
        # 关系数据库统�?        with calculator.db_manager.get_session() as session:
