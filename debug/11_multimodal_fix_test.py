...
def test_multimodal_stats(base_url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✗ 获取多模态统计信息失败: {response.status_code}")
            if response.text:
                print(f"  错误详情: {response.text}")
            return False
    except requests.exceptions.RequestException as e:
        print(f"✗ 请求多模态统计信息失败: {e}")
        return False
...
def test_multimodal_search(base_url):
    ...
    for query in test_queries:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✓ 查询 '{query}' 成功，返回 {len(results)} 个结果")
                
                # 显示第一个结果的详细信息（如果有）
                if results:
                    first_result = results[0]
                    print(f"  - 第一个结果综合得分: {first_result.get('combined_score', 0):.4f}")
    ...
    print(f"\n📊 多模态搜索测试结果: {success_count}/{len(test_queries)} 成功")
...
def test_semantic_search_comparison(base_url):
    ...
    try:
        ...
        print(f"✓ 多模态搜索: {'成功' if multimodal_success else '失败'}")
...
def main():
    ...
    # 测试步骤
    tests = [
        ("服务器状态", lambda: test_server_status(base_url)),
        ("多模态统计", lambda: test_multimodal_stats(base_url)),
        ("多模态搜索", lambda: test_multimodal_search(base_url)),
