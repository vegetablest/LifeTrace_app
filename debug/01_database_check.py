...
def check_database_status():
    ...
    try:
        with db_manager.get_session() as session:
            # 检查截图数�?            screenshot_count = session.query(Screenshot).count()
            ...
            for i, ocr in enumerate(recent_ocr, 1):
                ...
                print(f"  {i}. ID: {ocr.id}, 截图ID: {ocr.screenshot_id}, 置信�? {ocr.confidence}")
...
def main():
    """主函�?""
    print("🚀 LifeTrace 数据库状态检�?)
    print("=" * 50)
    
    # 检查基础数据�?    screenshot_count, ocr_count = check_database_status()
    ...
    if screenshot_count == 0:
        ...
        print(f"�?数据库状态正�? {screenshot_count} 个截�? {ocr_count} 个OCR结果")
