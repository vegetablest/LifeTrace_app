...
def check_file_system():
    ...
    if lifetrace_dir.exists():
        ...
        logger.info(f"📁 LifeTrace目录不存在，将自动创建: {lifetrace_dir}")
...
def test_database_initialization():
    ...
    try:
        ...
        with db_manager.get_session() as session:
            ...
            logger.info(f"📊 数据库状态:")
            logger.info(f"   截图记录数: {screenshot_count}")
            logger.info(f"   OCR记录数: {ocr_count}")
...
def test_vector_service():
    ...
    try:
        ...
        if vector_service:
            ...
            # 检查向量数据库状态
            if hasattr(vector_service, 'vector_db') and vector_service.vector_db:
                ...
                if collection:
                    ...
                    if doc_count == 0:
                        ...
                        logger.warning(f"⚠️ 基本向量数据库不为空: {doc_count} 个文档")
        else:
            logger.info("ℹ️ 基本向量服务未启用")
...
def test_multimodal_vector_service():
    """测试多模态向量服务"""
    logger.info("=== 测试多模态向量服务 ===")
    
    try:
        # 检查多模态配置
        multimodal_enabled = config.get('multimodal.enabled', False)
        logger.info(f"📋 多模态服务配置: {multimodal_enabled}")
        ...
        # 创建多模态向量服务
        multimodal_service = MultimodalVectorService()
        logger.info("✅ 多模态向量服务创建成功")
        ...
        if multimodal_service.text_vector_db:
            ...
            if text_collection:
                ...
                if text_count == 0:
                    ...
                    logger.warning(f"⚠️ 文本向量数据库不为空: {text_count} 个文档")
        ...
        if multimodal_service.image_vector_db:
            ...
            if image_collection:
                ...
                if image_count == 0:
                    ...
                    logger.warning(f"⚠️ 图像向量数据库不为空: {image_count} 个文档")
        
        return True
        
    except Exception as e:
        logger.error(f"❌ 多模态向量服务测试失败: {e}")
        return False
    ...
    try:
        ...
        if test_result:
            ...
            if search_results and len(search_results) > 0:
                logger.info("✅ 向量数据库搜索测试成功")
            else:
                logger.warning("⚠️ 向量数据库搜索测试失败")
        else:
            logger.warning("⚠️ 向量数据库写入测试失败")
...
def main():
    ...
    print("验证数据库重置是否成功，并测试系统功能")
    print()
    
    test_results = []
    
    # 1. 检查文件系统状态
    test_results.append(check_file_system())
    ...
    # 4. 测试多模态向量服务
    test_results.append(test_multimodal_vector_service())
