...
def check_reset_status():
    ...
    for vector_dir in vector_dirs:
        ...
        if vector_path.exists():
            print(f"⚠️ {vector_dir} 目录仍存在")
        else:
            print(f"✅ {vector_dir} 目录已删除")
    ...
    if screenshots_dir.exists():
        ...
        if total_files == 0:
            ...
            print(f"⚠️ 截图目录中仍有 {total_files} 个文件")
    else:
        print("✅ 截图目录不存在")
    ...
    if project_logs.exists():
        ...
        if len(log_files) == 0:
            ...
            print(f"ℹ️ 项目日志目录中有 {len(log_files)} 个文件")
    else:
        print("✅ 项目日志目录不存在")
    ...
    if user_logs.exists():
        ...
        if len(log_files) == 0:
            ...
            print(f"ℹ️ 用户日志目录中有 {len(log_files)} 个文件")
    else:
        print("✅ 用户日志目录不存在")
    ...
    for db_file in db_files[:3]:  # 只检查主要的数据库文件
        if (lifetrace_dir / db_file).exists():
