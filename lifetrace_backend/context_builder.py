...
class ContextBuilder:
    ...
    def build_context(self, query: str, retrieved_data: List[Dict[str, Any]], 
    ...
    def build_statistics_context(self, query: str, retrieved_data: List[Dict[str, Any]], 
        ...
        if hasattr(query_conditions, 'app_names'):
            # QueryConditions对象
            has_conditions = (query_conditions.app_names or query_conditions.keywords or 
                             query_conditions.start_date or query_conditions.end_date)
            if has_conditions:
                context_parts.append("\n查询条件:")
            ...
            # 字典格式
            has_conditions = (query_conditions.get('app_names') or query_conditions.get('keywords') or 
                             query_conditions.get('start_date') or query_conditions.get('end_date'))
            if has_conditions:
                context_parts.append("\n查询条件:")
    ...
    def _group_by_app(self, retrieved_data: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """按应用分组"""
        app_groups = {}
    ...
    def _truncate_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while len(json.dumps(context, ensure_ascii=False)) > self.max_context_length and detailed_records:
