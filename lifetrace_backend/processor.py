...
class FileProcessor:
    ...
    def start(self):
        ...
        for i in range(self.max_workers):
            thread = threading.Thread(target=self._worker_thread, name=f"FileWorker-{i}")
            thread.daemon = True
            thread.start()
...
# 动态添加方法到数据库管理器
db_manager.get_screenshot_by_path = get_screenshot_by_path.__get__(db_manager, type(db_manager))
