    ...
    def log_token_usage(self, 
        ...
        try:
            ...
            if user_query:
                ...
                usage_record['user_query_preview'] = user_query[:100] + ('...' if len(user_query) > 100 else '')
            ...
            # 同时记录到标准日志
            self.logger.info(f"Token usage - Model: {model}, Input: {input_tokens}, Output: {output_tokens}, Total: {input_tokens + output_tokens}")
                    ...
                    with open(log_file, 'r', encoding='utf-8') as f:
                        for line in f:
                            try:
                                ...
                                self.logger.warning(f"Failed to parse log line: {line.strip()}, error: {e}")
                                continue
