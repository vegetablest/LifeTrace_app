...
class MultimodalVectorService:
    ...
    def _build_metadata(self, ocr_result: OCRResult, screenshot: Screenshot, modality: str) -> Dict[str, Any]:
        """构建元数据"""
        metadata = {
            "modality": modality,
    ...
    def _search_text_with_embedding(self, query_embedding: np.ndarray, top_k: int,
                                  filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """使用预计算的嵌入搜索文本向量"""
        try:
            # 直接使用向量搜索文本数据库
    ...
    def _search_image_with_text(self, query_embedding: np.ndarray, top_k: int, 
                               filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """使用文本嵌入搜索图像向量"""
        try:
            # 直接使用向量搜索图像数据库
    ...
    def _merge_multimodal_results(self, text_results: List[Dict], image_results: List[Dict],
                                 text_weight: float, image_weight: float, top_k: int) -> List[Dict[str, Any]]:
        """合并多模态搜索结果"""
        try:
            # 构建结果字典，按OCR结果ID分组
            merged = {}
    ...
    def _enhance_result_data(self, result_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """增强结果数据，添加数据库信息"""
        try:
