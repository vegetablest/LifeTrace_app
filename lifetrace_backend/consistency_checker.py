            ...
            with db_manager.get_session() as session:
                for file_path in orphaned_files:
                    try:
                        ...
                        # 统计要删除的关联记录数
                        ocr_count = session.query(OCRResult).filter_by(screenshot_id=screenshot_id).count()
                        index_count = session.query(SearchIndex).filter_by(screenshot_id=screenshot_id).count()
                        queue_count = session.query(ProcessingQueue).filter_by(screenshot_id=screenshot_id).count()
...
class AdvancedConsistencyChecker(ConsistencyChecker):
    ...
    def __init__(self, check_interval: int = 300, vector_sync_interval: int = 1800):  # 向量同步30分钟一次
    ...
    def perform_consistency_check(self) -> dict:
        ...
        # 检查是否需要同步向量数据库
        now = datetime.now()
        if (self.last_vector_sync is None or 
            (now - self.last_vector_sync).total_seconds() >= self.vector_sync_interval):
