...
class HeartbeatLogger:
    ...
    def _heartbeat_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                self.logger.info(f"HEARTBEAT: {json.dumps(heartbeat_data, ensure_ascii=False)}")
    ...
    def _rotate_log_if_needed(self):
        """检查并轮转日志文件"""
        try:
            if self.heartbeat_file.exists() and self.heartbeat_file.stat().st_size > self.max_log_size:
                self._rotate_logs()
        except Exception as e:
            print(f"日志轮转检查失败: {e}")
    ...
    def _rotate_logs(self):
        ...
        try:
            ...
            backup_file = self.log_dir / f"{self.service_name}_heartbeat_{timestamp}.log"
    ...
    def reset_log(self):
        ...
        try:
            if self.heartbeat_file.exists():
                # 备份当前日志
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                backup_file = self.log_dir / f"{self.service_name}_heartbeat_reset_{timestamp}.log"
                self.heartbeat_file.rename(backup_file)
        ...
        for log_file in heartbeat_files:
            ...
            try:
                ...
                if last_heartbeat:
                    ...
                    # 更新服务状态
                    previous_status = self.service_status.get(service_name, {}).get('status')
