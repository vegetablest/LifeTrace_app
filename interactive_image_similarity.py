...
class InteractiveImageSimilarity:
    ...
    def process_query(self, query: str):
        ...
        # 计算相似�?        results = self.calculator.calculate_similarities(query, limit)
    ...
    def get_result_limit(self) -> int:
        """获取结果数量限制"""
        try:
            limit_input = input("显示多少个结果？(默认5个，按回车使用默认�?: ").strip()
            if not limit_input:
                return 5
            limit = int(limit_input)
            return max(1, min(limit, 50))  # 限制�?-50之间
        except ValueError:
            print("输入无效，使用默认�?")
            return 5
    ...
    def display_results(self, results: List[Dict[str, Any]], query: str):
        ...
        for i, result in enumerate(results, 1):
            ...
            print(f"    相似�? {similarity:.4f} {level}")
            print(f"    应用: {app_name}")
            if window_title != 'N/A':
        ...
        # 询问是否查看详细信息
        if len(results) > 0:
            detail_input = input("查看某个结果的详细信息？输入序号(1-{})，或按回车跳�? ".format(len(results))).strip()
    ...
    def display_detailed_result(self, result: Dict[str, Any], index: int):
        ...
        print(f"相似�? {result['similarity']:.6f}")
    ...
    def print_stats(self):
        ...
        try:
            # 获取图像数据库统�?            if self.calculator.multimodal_service.image_vector_db:
                collection = self.calculator.multimodal_service.image_vector_db.collection
                total_images = collection.count()
                print(f"图像数据�? {total_images} 个图�?)
            else:
                print("图像数据�? 未初始化")
            
            # 获取数据库统�?            with self.calculator.db_manager.get_session() as session:
