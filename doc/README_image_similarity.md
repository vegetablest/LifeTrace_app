...
- 🔍 **多模态语义搜索**：基于CLIP模型理解文本和图像的语义关系
- 📊 **相似度计算**：使用改进的相似度公式处理各种距离值
- 📋 **详细信息展示**：显示图像路径、OCR文本、应用程序、窗口标题等
- ⚡ **高性能**：平均查询时间约0.03秒
- 🎯 **灵活控制**：支持限制返回数量和详细信息显示
...
```bash
# 搜索与"电脑"相关的图像
python text_to_image_similarity.py "电脑" --limit 3 --details
```
...
请输入查询文本 (输入 'quit' 或 'exit' 退出): 电脑

🔍 搜索: 电脑
--------------------------------------------------
显示多少个结果？(默认5个，按回车使用默认值): 3
...
📊 找到 3 个相关图像:
 ...
 1. 📷 OCR ID: 1
    ...
    文本: 回636.4万 00:40 【独家】 《灵笼第二季》第12集预告 【5月国创】...
...
2. **向量搜索**：基于Chroma向量数据库
3. **相似度计算**：使用公式 `1.0 / (1.0 + distance)`
4. **数据增强**：结合关系数据库的元数据
