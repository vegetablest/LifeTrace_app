...
def reset_sqlite_database_force():
    ...
    try:
        ...
        logger.info(f"SQLite数据库重置完成，成功删除 {success_count} 个文件")
        return True
        
    except Exception as e:
        logger.error(f"强制重置SQLite数据库失败: {e}")
        return False
...
def reset_vector_databases_force():
    ...
    try:
        ...
        logger.info(f"向量数据库重置完成，成功删除 {success_count} 个目录")
        return True
        
    except Exception as e:
        logger.error(f"强制重置向量数据库失败: {e}")
        return False

def reset_screenshots_force():
    """强制删除截图文件"""
    try:
        logger.info("开始强制删除截图文件...")
        ...
        logger.info(f"截图文件删除完成，删除了 {deleted_count} 个文件")
        return True
...
def main():
    """主函数"""
    print("=== LifeTrace 强制数据库重置工具 ===")
    print("⚠️  强制删除所有数据，包括被占用的文件")
    print()
    
    logger.info("开始强制重置过程...")
    
    success_count = 0
    total_steps = 4
    
    # 1. 强制重置SQLite数据库
    if reset_sqlite_database_force():
        success_count += 1
    ...
    # 3. 强制重置向量数据库
    if reset_vector_databases_force():
        success_count += 1
